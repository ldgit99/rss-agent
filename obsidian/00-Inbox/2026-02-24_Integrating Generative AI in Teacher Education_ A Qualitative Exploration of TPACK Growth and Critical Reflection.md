# Integrating Generative AI in Teacher Education: A Qualitative Exploration of TPACK Growth and Critical Reflection

- Source: eric
- ID: eric:EJ1482950
- DOI: 명시되지 않음
- Published: 명시되지 않음
- Authors: Min Jou, Tzu-Hsuan Kuo, Yu-Chun Chiang, Yungwei Hao, Chun-Chiang Huang
- Categories: ERIC
- Link: https://eric.ed.gov/
- PDF: 명시되지 않음
- Keyword Score: 32

## Abstract
This study investigates how generative AI technologies influence pre-service teachers' pedagogical thinking and instructional design practices within a vocational education context. Drawing on a qualitative framework, the research engaged students in a task-based learning environment that integrated tools such as ChatGPT and image generators into authentic teaching design tasks. Data were collected through reflective journals, interviews, and teaching artifacts. Thematic analysis revealed three core trajectories of professional growth: (1) a shift from uncertainty to confidence in using AI tools; (2) the situated development of TPACK through iterative design and reflection; and (3) the emergence of critical awareness regarding AI ethics, accuracy, and bias. Students not only explored how AI could support their instructional creativity, but also expressed concerns about content reliability and the limitations of automated outputs. Their reflections illustrated an evolving understanding of AI not just as a tool, but as a coparticipant in instructional reasoning. The findings suggest that meaningful integration of generative AI requires more than technical training; it calls for pedagogical framing, ethical discourse, and reflective space. Teacher education programs must therefore cultivate not only AI fluency, but also critical and adaptive instructional mindsets capable of navigating the complexities of AI-supported teaching.

## OpenAI Review
---
title: Generative AI 통합을 통한 교사 교육: TPACK 성장과 비판적 반성의 질적 탐색
authors: Min Jou, Tzu-Hsuan Kuo, Yu-Chun Chiang, Yungwei Hao, Chun-Chiang Huang
year: 명시되지 않음
journal: 명시되지 않음
doi: 명시되지 않음
research_type: 질적 연구
methodology: 주제 분석
data_type: 반성 저널, 인터뷰, 교수 자료
sample: 예비 교사
context: 직업 교육
theoretical_framework: TPACK
keywords: [human-ai collaboration, classroom ai, ai tutoring, instructional design, scaffolding, tpack, ai literacy, teacher ai, education, learning analytics, llm classroom, ai pedagogy]
---

# APA 7th style

# Summary
이 연구는 생성적 AI 기술이 예비 교사들의 교수적 사고 및 수업 설계 관행에 미치는 영향을 조사하였다. 질적 프레임워크를 바탕으로, 연구는 ChatGPT와 이미지 생성기와 같은 도구를 통합한 과제 기반 학습 환경에서 학생들을 참여시켰다. 데이터는 반성 저널, 인터뷰, 교수 자료를 통해 수집되었으며, 주제 분석을 통해 세 가지 주요 성장 경로가 드러났다: (1) AI 도구 사용에 대한 불확실성에서 자신감으로의 전환; (2) 반복적 설계와 반성을 통한 TPACK의 상황적 발전; (3) AI 윤리, 정확성 및 편향에 대한 비판적 인식의 출현. 학생들은 AI가 교수적 창의성을 지원할 수 있는 방법을 탐구했지만, 콘텐츠 신뢰성과 자동화된 출력의 한계에 대한 우려도 표현하였다. 이들의 반성은 AI를 단순한 도구가 아닌 교수적 추론의 공동 참여자로 이해하는 진화하는 과정을 보여주었다. 연구 결과는 생성적 AI의 의미 있는 통합이 기술적 훈련 이상의 것을 요구하며, 교수적 프레이밍, 윤리적 담론 및 반성적 공간이 필요함을 시사한다. 따라서 교사 교육 프로그램은 AI 유창성뿐만 아니라 AI 지원 교수의 복잡성을 탐색할 수 있는 비판적이고 적응적인 교수적 사고 방식을 배양해야 한다.

# Research Logic
## Problem
예비 교사들이 생성적 AI를 효과적으로 활용하는 데 필요한 교수적 사고와 설계 능력의 부족.
## Theory
TPACK 이론을 기반으로 한 교수적 기술과 AI 통합의 필요성.
## Design
과제 기반 학습 환경에서 AI 도구를 활용한 질적 연구 설계.
## Findings
AI 도구 사용에 대한 자신감 증가, TPACK의 발전, AI 윤리에 대한 비판적 인식의 출현.
## Conclusion
교사 교육 프로그램은 AI 유창성과 비판적 사고를 동시에 배양해야 함.

# Key Findings
- AI 도구 사용에 대한 불확실성에서 자신감으로의 전환이 이루어졌다.
- 반복적 설계와 반성을 통해 TPACK이 상황적으로 발전하였다.
- AI 윤리, 정확성 및 편향에 대한 비판적 인식이 나타났다.

# Contributions
## Theoretical
TPACK 이론의 적용을 통해 AI 통합의 필요성을 강조.
## Methodological
질적 연구 방법을 통해 예비 교사의 경험을 심층적으로 탐구.
## Practical
AI 도구의 효과적인 통합을 위한 교수적 프레이밍과 반성적 공간의 필요성을 제시.

# Limitations
- 연구의 샘플 크기와 맥락이 제한적일 수 있다.
- 데이터 수집 방법의 다양성이 부족할 수 있다.

# Transferable Insights
- AI 도구의 효과적인 활용을 위해서는 기술적 훈련뿐만 아니라 교수적 사고가 필요하다.
- 교사 교육에서 AI 윤리와 비판적 사고의 중요성을 강조해야 한다.

# Idea Seeds
1. AI 도구 사용에 대한 자신감을 높이기 위한 구체적인 교육 프로그램 개발.
2. TPACK의 발전을 촉진하기 위한 반복적 설계 워크숍 운영.
3. AI 윤리에 대한 교육을 포함한 커리큘럼 개발.

# Citation Snippets
> Jou, M., Kuo, T.-H., Chiang, Y.-C., Hao, Y., & Huang, C.-C. (명시되지 않음). Integrating Generative AI in Teacher Education: A Qualitative Exploration of TPACK Growth and Critical Reflection.

tags: #human-ai collaboration, #classroom ai, #ai tutoring, #instructional design, #scaffolding, #tpack, #ai literacy, #teacher ai, #education, #learning analytics, #llm classroom, #ai pedagogy

---

요약: 이 연구는 생성적 AI가 예비 교사들의 교수적 사고와 수업 설계에 미치는 영향을 질적으로 탐구하였다. AI 도구 사용에 대한 자신감 증가, TPACK의 발전, AI 윤리에 대한 비판적 인식이 주요 발견으로 나타났다. 연구 결과는 교사 교육 프로그램에서 AI 유창성과 비판적 사고의 배양이 필요함을 강조한다.
